...
# 2. 「アクセス権」などの画面で、プログラムとして実行できるようにする
# 3. ファイルをダブルクリックし、確認画面で「実行」を選ぶ
# デスクトップ環境やファイルマネージャーによって、項目の名前や事前設定が異なる場合があります。

## ターミナルで実行する方法
# 1. このファイルを保存したフォルダーでターミナルを開く
# 2. [ファイル名].shを実際のファイル名に置き換えて、次のコマンドを実行する
#    chmod +x [ファイル名].sh
#    ./[ファイル名].sh
