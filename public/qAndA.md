...
設定がなにか問題を起こしている可能性があります。ソフトウェアを終了して設定ファイルを消去してもう一度起動してみてください。

設定ファイル（config.json）は、VOICEVOX ソフトウェア内の ヘルプ / お問い合わせ の「ログフォルダを表示」ボタンで確認できる logs フォルダと同じ階層にあります。
具体的には、以下の場所に保存されています。
