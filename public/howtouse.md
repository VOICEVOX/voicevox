...
マルチエンジン機能で追加したエンジンを削除する場合は、Finderのメニューバーの「移動」をクリックし、「フォルダへ移動」をクリックして表示された画面に「~/Library/Application Support/voicevox」と入力してEnterキーを押し、「vvpp-engines」フォルダを削除すれば完了です。
