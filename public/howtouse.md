...
**注記**: また、「セキュリティとプライバシー」環境設定の「一般」パネルで「このまま開く」ボタンをクリックすることで、ブロックされた「VOICEVOX」の例外を許可することもできます。
Macでこれらの環境設定を変更するには、アップルメニュー  ＞「システム環境設定」と選択して「セキュリティとプライバシー」 をクリックし、「一般」をクリックします。

**重要**：Apple Silicon製CPUのMacシリーズでは、「Rosetta 2」のインストールが必要です。「Rosetta 2」は初回起動時、未インストールの場合のみmacOSよりインストールの案内がありますので、案内に沿ってインストールしてください。
...
Mac版では初期状態ではショートカットが設定されていません。
お手数ですが手動で設定をお願いいたします。
...
**注記**:サードパーティーアプリ「[AppCleaner](https://freemacsoft.net/appcleaner/) 」で関連ファイルごと（隠しフォルダ内のものも含む）完全削除することもできます。

**警告**: そのアプリケーションは、次に手動で「ゴミ箱」を空にしたとき、またはFinderによって「ゴミ箱」が空にされたときに、Macから恒久的に削除されます。「VOICEVOX」で作成したファイルがある場合は、それらを開くことができなくなることがあります。「VOICEVOX」を残すことにした場合は、「ゴミ箱」を空にする前に戻してください。「ゴミ箱」でアプリケーションを選択し、「ファイル」＞「戻す」と選択します。
...
### Mac版
  ダウンロードした「VOICEVOX」を手動で「アプリケーション」フォルダーにコピーします。
...
**Mac版：**　Mac版ではGPUモードに対応してないためGPUモードでは起動しません。
