...
Apple Silicon製CPUのMacシリーズでは、「Rosetta 2」のインストールが必要です。「Rosetta 2」は初回起動時、未インストールの場合のみmacOSよりインストールの案内がありますので、案内に沿ってインストールしてください。
...
**警告**: そのアプリケーションは、次に手動で「ゴミ箱」を空にしたとき、またはFinderによって「ゴミ箱」が空にされたときに、Macから恒久的に削除されます。「VOICEVOX」を残すことにした場合は、「ゴミ箱」を空にする前に戻してください。「ゴミ箱」でアプリケーションを選択し、「ファイル」＞「戻す」と選択します。なお、「VOICEVOX」を再インストールすることで作成したプロジェクトファイルは引き続きご利用頂けます。
