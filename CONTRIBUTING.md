...
* コミットが終わったら、コードをコミュニティに提案しましょう。作成したコードをコミュニティへ提案する作業の事を「プルリクエスト」といいます。
...
* ドラフトプルリクエストは、進捗状況を共有するために使います。検討の歩みがわかるため、議論が必要な項目では特に有効な手段です。

* ドラフトプルリクエストの場合は、タイトルの先頭に `WIP:` をつける事が慣例になっています。

* 凡そ問題ないと判断できたら、プルリクエストを提出します。
この先は共同作業になるので、今一度作業忘れや問題がないか確認をしてください。

* プルリクエストを出すときには、関連するIssueのナンバーを記載しておきます。
...
* 議論する場所が分散する事を防ぐため、プルリクエストするタイミングで問題提起した方の番号を閉じましょう。コメントに下記のような表記をすることでIssue側の議論を閉じることができます。
