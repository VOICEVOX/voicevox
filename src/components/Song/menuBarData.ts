...
export const useMenuBarData = (store: Store): MaybeComputedMenuBarContent => {
  ...
  // 「表示」メニュー
  const [showSongCharacterPortrait, setShowSongCharacterPortrait] =
    useRootMiscSetting(store, "showSongCharacterPortrait");
  const viewSubMenuData = computed<MenuBarContent["view"]>(() => ({
    portrait: [
      {
        type: "button",
        label: showSongCharacterPortrait.value
          ? "立ち絵を非表示"
          : "立ち絵を表示",
        onClick: () => {
          setShowSongCharacterPortrait(!showSongCharacterPortrait.value);
        },
        disableWhenUiLocked: true,
      },
    ],
