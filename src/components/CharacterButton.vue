  ...
  >
    ...
    >
      <q-list style="min-width: max-content" class="character-item-container">
        ...
        <template
          v-for="(sectionCharacterInfos, sectionIndex) in props.characterInfos"
        >
          <q-separator v-if="sectionIndex > 0" :key="sectionIndex" />
          <q-item
            v-for="(characterInfo, characterIndex) in sectionCharacterInfos"
            :key="characterIndex"
            class="q-pa-none"
...
const selectedCharacter = computed(() => {
  ...
  const character = props.characterInfos
    .flat()
    .find(
      (characterInfo) =>
...
const getDefaultStyle = (speakerUuid: SpeakerId) => {
  ...
  const characterInfo = props.characterInfos
    .flat()
    .find((info) => info.metas.speakerUuid === speakerUuid);
