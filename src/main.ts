...
const themePlugin = createThemePlugin({
  type: "persistent",
  load: () => window.backend.getSetting("currentTheme"),
  save: async (themeSetting) => {
    await window.backend.setSetting("currentTheme", themeSetting);
  },
  setNativeTheme: (source) => window.backend.setNativeTheme(source),
});
